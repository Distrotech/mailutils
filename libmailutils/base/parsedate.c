...
union pd_yyalloc
{
  pd_yytype_int16 pd_yyss_alloc;
  YYSTYPE pd_yyvs_alloc;
};
...
# define YYSTACK_RELOCATE(Stack_alloc, Stack)				\
	...
	YYCOPY (&pd_yyptr->Stack_alloc, Stack, pd_yysize);			\
	Stack = &pd_yyptr->Stack_alloc;					\
...
#define pd_yypact_value_is_default(Yystate) \
  (!!((Yystate) == (-10)))

#define pd_yytable_value_is_error(Yytable_value) \
  YYID (0)
...
`------------------------------------------------------------------*/
     ...
     || defined __cplusplus || defined _MSC_VER)
static void
pd_yy_stack_print (pd_yytype_int16 *pd_yybottom, pd_yytype_int16 *pd_yytop)
#else
static void
pd_yy_stack_print (pd_yybottom, pd_yytop)
    pd_yytype_int16 *pd_yybottom;
    pd_yytype_int16 *pd_yytop;
#endif
{
  YYFPRINTF (stderr, "Stack now");
  for (; pd_yybottom <= pd_yytop; pd_yybottom++)
    {
      int pd_yybot = *pd_yybottom;
      YYFPRINTF (stderr, " %d", pd_yybot);
    }
  YYFPRINTF (stderr, "\n");
}
...
static int
pd_yysyntax_error (YYSIZE_T *pd_yymsg_alloc, char **pd_yymsg,
                pd_yytype_int16 *pd_yyssp, int pd_yytoken)
{
  YYSIZE_T pd_yysize0 = pd_yytnamerr (YY_NULL, pd_yytname[pd_yytoken]);
  YYSIZE_T pd_yysize = pd_yysize0;
  enum { YYERROR_VERBOSE_ARGS_MAXIMUM = 5 };
  /* Internationalized format string. */
  const char *pd_yyformat = YY_NULL;
  /* Arguments of pd_yyformat. */
  ...
  */
  if (pd_yytoken != YYEMPTY)
    {
      int pd_yyn = pd_yypact[*pd_yyssp];
      pd_yyarg[pd_yycount++] = pd_yytname[pd_yytoken];
      if (!pd_yypact_value_is_default (pd_yyn))
        {
          ...
          for (pd_yyx = pd_yyxbegin; pd_yyx < pd_yyxend; ++pd_yyx)
            if (pd_yycheck[pd_yyx + pd_yyn] == pd_yyx && pd_yyx != YYTERROR
                && !pd_yytable_value_is_error (pd_yytable[pd_yyx + pd_yyn]))
              {
                ...
                {
                  YYSIZE_T pd_yysize1 = pd_yysize + pd_yytnamerr (YY_NULL, pd_yytname[pd_yyx]);
                  if (! (pd_yysize <= pd_yysize1
                         && pd_yysize1 <= YYSTACK_ALLOC_MAXIMUM))
                    return 2;
...
# undef YYCASE_
  ...
  {
    YYSIZE_T pd_yysize1 = pd_yysize + pd_yystrlen (pd_yyformat);
    if (! (pd_yysize <= pd_yysize1 && pd_yysize1 <= YYSTACK_ALLOC_MAXIMUM))
      return 2;
    pd_yysize = pd_yysize1;
  }

  if (*pd_yymsg_alloc < pd_yysize)
    {
      *pd_yymsg_alloc = 2 * pd_yysize;
      if (! (pd_yysize <= *pd_yymsg_alloc
             && *pd_yymsg_alloc <= YYSTACK_ALLOC_MAXIMUM))
        *pd_yymsg_alloc = YYSTACK_ALLOC_MAXIMUM;
      return 1;
    }
  ...
  {
    char *pd_yyp = *pd_yymsg;
    int pd_yyi = 0;
    while ((*pd_yyp = *pd_yyformat) != '\0')
      if (*pd_yyp == '%' && pd_yyformat[1] == 's' && pd_yyi < pd_yycount)
        {
...
YYSTYPE pd_yylval YY_INITIAL_VALUE(pd_yyval_default);
...
#endif
{
    ...
    /* The stacks and their tools:
       ...
       Refer to the stacks through separate pointers, to allow pd_yyoverflow
...
#define YYPOPSTACK(N)   (pd_yyvsp -= (N), pd_yyssp -= (N))
  ...
  pd_yyssp = pd_yyss = pd_yyssa;
  pd_yyvsp = pd_yyvs = pd_yyvsa;
  pd_yystacksize = YYINITDEPTH;
...
# else
	...
	if (! pd_yyptr)
	  goto pd_yyexhaustedlab;
	YYSTACK_RELOCATE (pd_yyss_alloc, pd_yyss);
	YYSTACK_RELOCATE (pd_yyvs_alloc, pd_yyvs);
...
pd_yybackup:
  ...
  if (pd_yyn <= 0)
    {
      if (pd_yytable_value_is_error (pd_yyn))
        goto pd_yyerrlab;
      pd_yyn = -pd_yyn;
      goto pd_yyreduce;
    }
...
/* Line 1792 of yacc.c  */
#line 2174 "parsedate.c"
      default: break;
    }
  /* User semantic actions sometimes alter pd_yychar, and that requires
     that pd_yytoken be updated with the new translation.  We take the
     approach of translating immediately before every use of pd_yytoken.
...
pd_yyerrlab:
  ...
  pd_yytoken = pd_yychar == YYEMPTY ? YYEMPTY : YYTRANSLATE (pd_yychar);
...
#else
# define YYSYNTAX_ERROR pd_yysyntax_error (&pd_yymsg_alloc, &pd_yymsg, \
                                        pd_yyssp, pd_yytoken)
      {
        char const *pd_yymsgp = YY_("syntax error");
        int pd_yysyntax_error_status;
        pd_yysyntax_error_status = YYSYNTAX_ERROR;
        if (pd_yysyntax_error_status == 0)
          pd_yymsgp = pd_yymsg;
        else if (pd_yysyntax_error_status == 1)
          {
            if (pd_yymsg != pd_yymsgbuf)
              YYSTACK_FREE (pd_yymsg);
            pd_yymsg = (char *) YYSTACK_ALLOC (pd_yymsg_alloc);
            if (!pd_yymsg)
              {
                pd_yymsg = pd_yymsgbuf;
                pd_yymsg_alloc = sizeof pd_yymsgbuf;
                pd_yysyntax_error_status = 2;
              }
            else
              {
                pd_yysyntax_error_status = YYSYNTAX_ERROR;
        ...
        pd_yyerror (pd_yymsgp);
        if (pd_yysyntax_error_status == 2)
          goto pd_yyexhaustedlab;
      }
